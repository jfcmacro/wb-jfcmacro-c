...
main(int argc, char* argv[])
{
  ...
  if (setsockopt(s, SOL_SOCKET, SO_REUSEADDR, &onoff, sizeof(int)) < 0) {
    sprintf(out, "No reutilizar el socket %s\n",
	    strerror(errno));
    printError(out, errno, 1);
  }

  inet_aton(DIR_MULTICAST, &maddr);
  ...
  if (setsockopt(s, IPPROTO_IP, IP_MULTICAST_LOOP, &onoff, sizeof(int)) < 0) {
    sprintf(out, "No pudo re-obtener los mensajes enviados %s %d\n",
	    strerror(errno));
    printError(out, errno, 1);
  }
  ...
  if (setsockopt(s, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq, sizeof(mreq)) < 0) {
    sprintf(out, "No pudo matricular la membresia %s\n",
	    strerror(errno));
    printError(out, errno, 1);
  }
